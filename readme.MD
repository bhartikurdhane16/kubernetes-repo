...
kubectl get namespace                    # listout all namespace, there are 4 name spaces alreay presents,
...
kube-public: This created automatically and is accessible to all users (including those not authenticated). It's primarily used for storing resources that should be accessible to all users, such as cluster-wide configuration or informational data.
kube-system: This namespace contains Kubernetes system resources and components that are essential for the functioning of the cluster. It includes components like kube-proxy, kube-dns, kube-scheduler, kube-controller-manager, and others.
