...
## as we know we have single node cluster /minikube cluster so we can not access pod from outside the cluster(from host)  but access from inside the cluster for that need to take ssh so to access app need to enter in cluster that need to take ssh to enter in minikube 
