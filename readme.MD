...
## as we know we have single node cluster /minikube cluster so we can not access pod from outside the cluster(from host) for that need to take ssh to enter in minikube 
