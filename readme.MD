...
## as we know we have single node cluster /minikube cluster so we can not access pod from outside the cluster(from host)  but access from inside the cluster so to enter in cluster then nedd to take ssh of cluster. 
...
# if wanted to access same pod from outside 